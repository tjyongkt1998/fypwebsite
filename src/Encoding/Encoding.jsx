import React, { Component, Fragment } from "react";
import { Row, Col, Typography, Divider } from "antd";
import { withRouter } from "react-router-dom";
import "../Encoding/Encoding.css";

const { Text } = Typography;

class Encoding extends Component {
  render() {
    return (
      <Fragment>
        <Row
          style={{
            textAlign: "center",
            marginBottom: "80px",
          }}
        >
          <Col xs={1} md={2} lg={4} style={{ backgroundColor: "#F0F0F0" }} />
          <Col
            xs={22}
            md={20}
            lg={16}
            style={{ paddingRight: "20px", paddingLeft: "20px" }}
          >
            <Col span={24} className="homepage-col">
              <Text
                style={{
                  textAlign: "center",
                  fontSize: "50px",
                  fontFamily: "lato-bold",
                  marginTop: "20px",
                }}
              >
                Encoding
              </Text>
            </Col>

            <Col span={24}>
              <br />
              <br />
              <Text underline>
                <h2>Overview</h2>
              </Text>
              <br />
              <br />
            </Col>

            <Row style={{}}>
              <Col span={2} />
              <Col span={8}>
                {" "}
                <img
                  src="/assets/encoding.png"
                  alt="topicsEncoding"
                  className="topics-Encoding-Image"
                />
              </Col>
              <Col span={12}>
                <Text className="encoding-intro">
                  Unicode/Encoding has goal of overcoming the limitations of
                  traditional character encodings.
                  <br />
                  <br />
                </Text>
              </Col>
              <Col span={2} />
            </Row>

            <Divider />

            <Col span={24}>
              <br />
              <br />
              <Text underline>
                <h2>Concepts</h2>
              </Text>
              <br />
              <br />
            </Col>

            <Row style={{}}>
              <Col span={2} />
              <Col span={8}>
                {" "}
                <img
                  src="/assets/encoding.png"
                  alt="topicsEncoding"
                  className="topics-Encoding-Image"
                />
              </Col>
              <Col span={12}>
                <Text className="encoding-intro">
                  Learn about core concepts of localisation and
                  internationalisation! These concepts will help you have a
                  better understanding of the each respective topic and when to
                  apply these topics according to required situations in
                  localisation!
                  <br />
                  <br />
                </Text>
              </Col>
              <Col span={2} />
            </Row>

            <Divider />

            <Col span={24}>
              <br />
              <br />
              <Text underline>
                <h2>Challenges</h2>
              </Text>
              <br />
              <br />
            </Col>

            <Row style={{}}>
              <Col span={2} />
              <Col span={8}>
                {" "}
                <img
                  src="/assets/encoding.png"
                  alt="topicsEncoding"
                  className="topics-Encoding-Image"
                />
              </Col>
              <Col span={12}>
                <Text className="encoding-intro">
                  Learn about core concepts of localisation and
                  internationalisation! These concepts will help you have a
                  better understanding of the each respective topic and when to
                  apply these topics according to required situations in
                  localisation!
                  <br />
                  <br />
                </Text>
              </Col>
              <Col span={2} />
            </Row>

            <Divider />

            <Col span={24}>
              <br />
              <br />
              <Text underline>
                <h2>Interactivity</h2>
              </Text>
              <br />
              <br />
            </Col>

            <Row style={{}}>
              <Col span={2} />
              <Col span={8}>
                {" "}
                <img
                  src="/assets/encoding.png"
                  alt="topicsEncoding"
                  className="topics-Encoding-Image"
                />
              </Col>
              <Col span={12}>
                <Text className="encoding-intro">
                  Learn about core concepts of localisation and
                  internationalisation! These concepts will help you have a
                  better understanding of the each respective topic and when to
                  apply these topics according to required situations in
                  localisation!
                  <br />
                  <br />
                </Text>
              </Col>
              <Col span={2} />
            </Row>
            <Divider />
          </Col>
          <Col xs={1} md={2} lg={4} style={{ backgroundColor: "#F0F0F0" }} />
        </Row>
      </Fragment>
    );
  }
}

export default (withRouter, Encoding);
